...
@section('contents')
    <div class="content content-components">
        <div class="container">
            ...
            <h4 class="mg-b-20">Sektor/Bidang Statistik</h4>
            <hr>
            ...
            <table class="table table-striped table-hover">
                <thead class="thead-dark">
                    <tr>
                        ...
                        <th colspan="2">
                            @can('bidang-create')
                                <a class="btn btn-icon btn-primary btn-xs" href="{!! route('sektor-bidang.statistik.create', @$code) !!}"><i
                                        class="fa fa-plus"></i></a>
                            @endcan
                            <div class="btn-group">
                                <a class="btn btn-light btn-xs" href="{!! route('sektor-bidang.statistik.grouping') !!}"> GROUPING BIDANG/SEKTOR</a>
                                <button type="button" class="btn btn-light btn-xs btn-icon" data-container="body"
                                    data-toggle="popover" data-placement="top"
                                    data-content="Pengelompokan/ Grouping Bidang/Sektor pada aplikasi untuk bisa di centang bersamaan pada aplikasi Otonometer."><i
                    ...
                    @foreach ($bidang as $bid)
                        <tr class="{{ $bid['cls_str'] }}" id="tr-{{ $bid['id'] }}">
                            ...
                            <td>
                                <div class="btn-group">
                                    @can('bidang-create')
                                        <a class="btn btn-icon btn-outline-primary btn-xs" href="{!! route('sektor-bidang.statistik.create', @$bid['id']) !!}"><i
                                                class="fa fa-plus"></i></a>
                                    @endcan
                                    @can('bidang-edit')
                                        <a href="{{ route('sektor-bidang.statistik.edit', $bid['id']) }}"
                                    ...
                                    @can('bidang-delete')
                                        {!! Form::open([
                                            'route' => ['sektor-bidang.statistik.destroy', $bid['id']],
                                            'method' => 'delete',
...
@section('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            $.ajax({
                url: "{!! route('sektor-bidang.check-tree', 3) !!}",
                method: 'GET',
                dataType: 'json',
                success: function(response) {
                    if (!response.valid) {
                        response.nonvalid.forEach(element => {
                            $("#tr-" + element + ">td:nth-child(4)").append(
