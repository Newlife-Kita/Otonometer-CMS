...
@section('contents')
    <div class="content content-components">
        <div class="container">
                ...
                <div>
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb breadcrumb-style1 mg-b-10">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Data Sektor/Bidang Ekonomi</li>
                        </ol>
                    </nav>
                </div>
            </div>
            ...
            <h4 class="mg-b-10">Data Sektor/Bidang Ekonomi</h4>
            ...
            <div class="d-sm-flex align-items-center justify-content-between mg-b-20 mg-lg-b-25 mg-xl-b-30">
                ...
                <div class="d-md-block">
                    <a class="btn btn-sm btn-dark btn-uppercase" href="{!! route('data-ekonomi.templateExcel') !!}"><i
                            class="fa fa-download"></i> Downoad Template Excel</a>

                    @can('bidangnilai-create')
                        <a class="btn btn-sm btn-primary btn-uppercase" href="{!! route('data-ekonomi.uploadform') !!}"><i
                                class="fa fa-upload"></i> Upload & Publish Data</a>
                    @endcan

                    @can('bidangnilai-delete')
                        <a class="btn btn-sm btn-danger btn-uppercase" href="{!! route('data-ekonomi.deleteform') !!}"><i
            ...
            <div class="bd-1 bg-gray-300 pd-10">
                <div class="input-group mg-b-10">
                    ...
                    <div class="input-group-append pd-l-15">
                        <span class="input-group-text">Sektor Ekonomi</span>
                    </div>
    ...
    <script>
        ...
        $(document).on('click', '.btn-download', function() {
            ...
            if (tahun < 1) {
                ...
                location.href = `{!! route('data-ekonomi.download') !!}?tahun=` + tahun;
            } else {
                location.href = `{!! route('data-ekonomi.download.province') !!}?tahun=` + tahun + `&province_id=` + provinceId;
            }
        })
        ...
        $(document).on('click', '.btn-update-delete', function() {
            ...
            if (bidang.length < 1) {
                Swal.fire({
                    text: 'Bidang/Sektor Ekonomi harus di pilih?',
                    icon: 'warning',
                })
                return;
            }
            ...
            $.ajax({
                method: "DELETE",
                url: "{!! route('data-ekonomi.deleteupdated') !!}",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                data: {
        ...
        $(document).on('click', '.check-update', function() {
            ...
            if (!isEveryIncluded) {
                $('#check-all-update').addClass('btn-success').removeClass(
                    'btn-outline-success');
                $("#check-all-update").find('i').removeClass('fa-times').addClass('fa-check');
                ...
                $('#check-all-update').addClass('btn-outline-success').removeClass(
                    'btn-success');
                $("#check-all-update").find('i').removeClass('fa-check').addClass('fa-times');
                $('#check-all-update').contents().filter(function() {
                    return this.nodeType === 3; // Filter text nodes
            ...
            if (listCheck.length > 0) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            method: "POST",
                            url: "{!! route('data-ekonomi.publishupdate') !!}",
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                            },
                            data: {
        ...
        $(document).on('click', '#delete-all-update', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        method: "DELETE",
                        url: "{!! route('data-ekonomi.deleteupdatedall') !!}",
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        },
                        data: {
        ...
        $(document).on('click', '.btn-filter', function() {
            ...
            $.ajax({
                url: "{!! route('data-ekonomi.getdata') !!}",
                data: {
                    tahun: tahun,
                    bidang: bidang
                },
                        ...
                        $.each(result.items, function(index, value) {
                            ...
                            html_body += `<tr style="width:100%">
                                ...
                                <td class=" tx-left pd-r-10" style="width:35%">` + (nominal === null ? 'NULL' :
                                    formatMoney(nominal)) +
                                `</td>
                                <td class=" tx-left pd-r-10" style="width:100%;display:flex; justify-content:space-between;"><span style="margin-right: 5px">` +
                                ...
                                value.id_wilayah +
                                `"><i class="fas fa-clipboard"></i> Catatan</button>
                                    <button class="btn-update btn btn-xs btn-outline-primary btn-icon" style="margin-right: 5px" id="` +
                                value
                                .id_wilayah +
