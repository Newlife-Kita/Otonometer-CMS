...
@section('contents')
    <div class="content content-components">
        <div class="container">
                ...
                <div>
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb breadcrumb-style1 mg-b-10">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Data Sektor/Bidang Keuangan</li>
                        </ol>
                    </nav>
                </div>
            </div>
            ...
            <h4 class="mg-b-10">Data Sektor/Bidang Keuangan</h4>
            ...
            <div class="d-sm-flex align-items-center justify-content-between mg-b-20 mg-lg-b-25 mg-xl-b-30">
                ...
                <div class="d-md-block">
                    <a class="btn btn-sm btn-dark btn-uppercase" href="{!! route('data-keuangan.templateExcel') !!}"><i
                            class="fa fa-download"></i> Downoad Template Excel</a>
                    @can('bidangnilai-create')
                        <a class="btn btn-sm btn-primary btn-uppercase" href="{!! route('data-keuangan.uploadform') !!}"><i
                                class="fa fa-upload"></i> Upload & Publish Data</a>
                    @endcan
                    @can('bidangnilai-delete')
                        <a class="btn btn-sm btn-danger btn-uppercase" href="{!! route('data-keuangan.deleteform') !!}"><i
            ...
            <div class="bd-1 bg-gray-300 pd-10">
                <div class="input-group mg-b-10">
                    ...
                    <div class="input-group-append pd-l-15">
                        <span class="input-group-text">Sektor Keuangan</span>
                    </div>
                                ...
                                <tfoot>
                                    <tr>
                                        <td colspan="{{ $bahasa->count() }}">
                                            <button class="btn btn-outline-primary" type="button" id="button-new-notes"><i
                                                    class="fas fa-plus"></i> Catatan</button>
    ...
    <script>
        ...
        $(document).on('click', '.btn-download', function() {
            ...
            if (tahun < 1) {
                ...
                location.href = `{!! route('data-keuangan.download') !!}?tahun=` + tahun;
            } else {
                location.href = `{!! route('data-keuangan.download.province') !!}?tahun=` + tahun + `&province_id=` + provinceId;
            }
        })
        ...
        $(document).on('click', '.btn-update-delete', function() {
            ...
            if (bidang.length < 1) {
                Swal.fire({
                    text: 'Bidang/Sektor Keuangan harus di pilih?',
                    icon: 'warning',
                })
                return;
            }
            ...
            $.ajax({
                method: "DELETE",
                url: "{!! route('data-keuangan.deleteupdated') !!}",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                data: {
        ...
        $(document).on('click', '.btn-update', async function() {
            ...
            if (nominal.value) {
                $.ajax({
                    ...
                    url: "{!! route('data-keuangan.updatedata') !!}",
                    data: {
                        tahun: tahun,
                        bidang: bidang,
                        wilayah: id,
        ...
        $(document).on('click', '.btn-filter', function() {
            ...
            $.ajax({
                url: "{!! route('data-keuangan.getdata') !!}",
                data: {
                    tahun: tahun,
                    bidang: bidang
                },
